...
func TestGeneralos(t *testing.T) {
	err := os.Chdir("../../../data")
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Run("NewGeneralOS_Success", func(t *testing.T) {
		ns, err := NewGeneralOS(clusterconfig)
		if err != nil {
	...
	t.Run("NewGeneralOS_NilConfig", func(t *testing.T) {
		ns, err := NewGeneralOS(nil)
		if err == nil {
	...
	t.Run("NewGeneralOS_EmptyMasterConfig", func(t *testing.T) {
		emptyMasterConfig := *clusterconfig
		emptyMasterConfig.Master = []asset.NodeAsset{}
		ns, err := NewGeneralOS(&emptyMasterConfig)
		if err == nil {
			t.Error("Expected error, got nil")
		}
		if ns != nil {
