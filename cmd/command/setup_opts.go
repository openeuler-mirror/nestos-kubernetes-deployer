...
func SetupTemplateCmdOpts(templateCmd *cobra.Command) {
	...
	flags.StringVarP(&opts.Opts.ClusterConfigFile, "file", "f", "template.yaml", "Location of cluster deploy config file")
	flags.StringVarP(&opts.Opts.ClusterID, "cluster-id", "", "cluster", "Cluster ID")
	flags.StringVarP(&opts.Opts.Platform, "platform", "", "libvirt", "Select the infrastructure platform to deploy the cluster")
	flags.StringArrayVarP(&opts.Opts.Master.Hostname, "master-hostname", "", []string{"k8s-master01", "k8s-master02", "k8s-master03"}, "Set master hostnames")
	flags.UintVar(&opts.Opts.Master.CPU, "master-cpu", 4, "Set master CPU (units: cores)")
	flags.UintVar(&opts.Opts.Master.RAM, "master-ram", 8, "Set master RAM (units: GB)")
	flags.UintVar(&opts.Opts.Master.Disk, "master-disk", 50, "Set master disk size (units: GB)")
	flags.StringVarP(&opts.Opts.Master.UserName, "master-username", "", "root", "User to login master node")
	flags.StringVarP(&opts.Opts.Master.Password, "master-password", "", "$1$yoursalt$UGhjCXAJKpWWpeN8xsF.c/", "Password to login master node")
	flags.StringVarP(&opts.Opts.Master.SSHKey, "master-sshkey", "", "", "Set master nodes ssh private key for authentication")
	flags.StringArrayVarP(&opts.Opts.Master.IP, "master-ips", "", []string{"192.168.1.1", "192.168.1.2", "192.168.1.3"}, "Set master IPs")
	flags.StringArrayVarP(&opts.Opts.Master.IgnFilePath, "master-igns", "", []string{}, "Set master nodes ignition file path")
	flags.StringArrayVarP(&opts.Opts.Worker.Hostname, "worker-hostname", "", []string{"k8s-worker01", "k8s-worker02", "k8s-worker03"}, "Set worker hostnames")
	flags.UintVar(&opts.Opts.Worker.CPU, "worker-cpu", 4, "Set worker CPU (units: cores)")
	flags.UintVar(&opts.Opts.Worker.RAM, "worker-ram", 8, "Set worker RAM (units: GB)")
	flags.UintVar(&opts.Opts.Worker.Disk, "worker-disk", 50, "Set worker disk size (units: GB)")
	flags.StringVarP(&opts.Opts.Worker.UserName, "worker-username", "", "root", "User to login worker node")
	flags.StringVarP(&opts.Opts.Worker.Password, "worker-password", "", "$1$yoursalt$UGhjCXAJKpWWpeN8xsF.c/", "Password to login worker node")
	flags.StringVarP(&opts.Opts.Worker.SSHKey, "worker-sshkey", "", "", "Set worker nodes ssh private key for authentication")
	flags.StringArrayVarP(&opts.Opts.Worker.IP, "worker-ips", "", []string{"192.168.1.4", "192.168.1.5", "192.168.1.6"}, "Set worker IPs")
	flags.StringArrayVarP(&opts.Opts.Worker.IgnFilePath, "worker-igns", "", []string{}, "Set worker nodes ignition file path")
	flags.StringVarP(&opts.Opts.ImageRegistry, "image-registry", "", "", "Specify the registry address for pulling the Kubernetes component container image")
	flags.StringVarP(&opts.Opts.PauseImage, "pause-image", "", "pause:3.6", "Specify the image for the pause container")
	flags.StringVarP(&opts.Opts.ReleaseImageUrl, "release-image-url", "", "", "Specify the URL of the NestOS container image that contains the Kubernetes component. Only supports the qcow2 format.")
	flags.StringVarP(&opts.Opts.KubeVersion, "kubeversion", "", "v1.23.10", "Specify the version of Kubernetes to deploy")
	flags.StringVarP(&opts.Opts.Token, "token", "", "", "Specify the authentication token for accessing resources")
	flags.StringVarP(&opts.Opts.CertificateKey, "CertificateKey", "", "", "Specifies the certificate key to be added to the master node")
	flags.StringVarP(&opts.Opts.NetWork.ServiceSubnet, "service-subnet", "", "10.96.0.0/16", "Specify the subnet for Kubernetes services")
	flags.StringVarP(&opts.Opts.NetWork.PodSubnet, "pod-subnet", "", "10.100.0.0/16", "Specify the subnet for Kubernetes Pods.")
	flags.StringVarP(&opts.Opts.NetWork.DNS.ImageVersion, "image-version", "", "v1.8.6", "Specify the version of the CoreDNS container image")
	flags.StringVarP(&opts.Opts.Housekeeper.ControllerImageUrl, "controller-image-url", "", "hub.oepkgs.net/nestos/nkd/amd64/housekeeper-controller-manager:0.1.0", "Specify the URL of the container image for the housekeeper controller component")
	flags.StringVarP(&opts.Opts.Housekeeper.OperatorImageUrl, "operator-image-url", "", "hub.oepkgs.net/nestos/nkd/amd64/housekeeper-operator-manager:0.1.0", "Specify the URL of the container image for the housekeeper operator component")
	flags.BoolVarP(&opts.Opts.DeployHousekeeper, "deploy-housekeeper", "", false, "Deploy the Housekeeper Operator.")
}
