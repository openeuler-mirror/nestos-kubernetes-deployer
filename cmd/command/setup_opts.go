...
func SetupDeployCmdOpts(deployCmd *cobra.Command) {
	...
	flags.StringVarP(&opts.Opts.ClusterConfigFile, "file", "f", "./deploy-config.yaml", "Location of cluster deploy config file")
	...
	flags.UintVar(&opts.Opts.Master.CPU, "master-cpu", 8, "Set master CPU (units: cores)")
	flags.UintVar(&opts.Opts.Master.RAM, "master-ram", 16, "Set master RAM (units: GB)")
